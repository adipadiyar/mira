...
\usepackage{amsmath,amssymb,amsthm} % For \text command
\usepackage{hyperref}
...
\subsection*{2A}
\begin{itemize}
    \item[1] LHS $\leq$ RHS by countable sub additivity. The other direction follows by inclusion
    \item[2]  $I_1,I_2,..$ are open intervals that cover A if and only if $tI_1,tI_2,..$ are open intervals that cover $tA$.
    \item[3]
...
\subsection*{2B}
\begin{itemize}
    \item [1] This is obvious
    \item [2] Simple verification
    \item [3] We show that $\mathcal{S}$ contains $(a,b)$. Let $a_1,a_2,..$ be rationals strictly decreasing to $a$ and $b_1,b_2,..$ be rationals strictly increasing to $b$. Then $\bigcup(a_i,b_i]$=$(a,b)$
    \item [4] We need to show $\mathcal{S}$ contains $(a,\infty)$. Let $a_1,a_2,..$ be rationals strictly decreasing to $a$. Let $n > a_1$ Then $\bigcup (a_i,n+i]=(a,\infty)$
    \item [5] $\mathcal{S}$ clearly contains $(r,\infty)$ for $r \in \mathbb{Q}$. Now taking strictly decreasing rationals gets $(a,\infty)$
    \item [6] Strictly decreasing rationals again
    \item [7] $x \to x+ t$ is measurable
    \item [8] $x \to tx$ is measurable
